...
## Project organization
The starter code is organized into a few small packages so you can focus on implementing algorithms while everything else stays consistent for everyone.

[Starter code →](https://github.com/evisp/dsa-algorithms/tree/main/labs/lab_sprint1/dsa/lab){ .md-button .md-button--primary }

- `dsa.lab`  
  Entry point (`Main`) where you control dataset types, input size (`n`), warmup runs, and measured runs.

- `dsa.lab.model`  
  The data item (`Record`) — all algorithms sort/search using `record.key()`.

- `dsa.lab.alg.sort` and `dsa.lab.alg.search`  
  **Your work area:** `StudentSorts` (sorting) and `StudentSearches` (searching).

- `dsa.lab.data`  
  Generates input datasets (random, nearly sorted, duplicates, reversed, sorted).

- `dsa.lab.bench`  
  Runs benchmarks, prints console progress, writes `results_summary.csv`.

- `dsa.lab.util`  
  Helpers: array copying, validation (e.g., “is sorted?”).

!!! warning "Rule for fair comparisons"
    Do not modify the benchmark/timing logic. Your comparison is about the algorithms, not changing the measurement setup.


## Your tasks
Implement the required algorithms **only** in the two student files so everyone runs under the same benchmark setup:

### Student Sorts `dsa.lab.alg.sort.StudentSorts`
...
### Student Searches `dsa.lab.alg.search.StudentSearches`
